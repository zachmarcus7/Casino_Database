/********************************************
* Function for sending an XMLHTTP request to Edit Supervisors
...
let flip = "flip3";
let port = 43029;
let page = "/Edit_Supervisors";
let redirect = "/Update_Supervisors";
...
function deleteButtons(){
    ...
    for(let i = 0; i < buttons.length; i++){
        buttons[i].addEventListener("click", function(event){
            let callType = "del";
            let supervisor_ID = "";
            supervisor_ID = "&supervisor_ID=" + buttons[i].value;
            let req = new XMLHttpRequest();
            let url = "http://" + flip + ".engr.oregonstate.edu:" + port + page + "/?callType=" + callType + supervisor_ID;
            req.open("GET", url);
...
function updateButtons(){
    ...
    for(let i = 0; i < buttons.length; i++){
        buttons[i].addEventListener("click", function(event){
            location.href = redirect + "/?supervisor_ID=" + buttons[i].value;
        });
    };
};
